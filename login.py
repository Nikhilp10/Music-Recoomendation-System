...
def add_bg_from_url():
  st.markdown(
    f"""
         <style>
         .stApp {{
             background-image: url("https://images4.alphacoders.com/850/85006.jpg");
...
SPOTIPY_CLIENT_ID = 'b52a3c431e8f44a3afee10a8261a85ce'
SPOTIPY_CLIENT_SECRET = '25e62057093f4107b3a9dcb7e789781c'
...
def main():
  ...
  if choice=="Home":
    st.subheader("Devloped by :-Nikhil Patil")
    st.markdown("""
                A music recommendation system and login interface using Streamlit is a project that combines machine learning and web development to create a music recommendation system with a user-friendly interface. The summary of this project should cover the essential aspects of the project, including its purpose, methodology, and results."""
                )
    ...
    if st.sidebar.checkbox("login"):
      ...
      if result:
        ...
        search_selected = option_menu(
          menu_title=None,
          options=['Song/Track', 'Artist', 'Album'],
          icons=["file-earmark-play-fill", "person-circle", "collection"],
          orientation="horizontal"
        )

        search_keyword = st.text_input(search_selected + " (Keyword Search)")
        button_clicked = st.button("Search", key="1")
        ...
        if selected_artist is not None and len(artists) > 0:
          ...
          if artist_id is not None:
            selected_artist_choice = option_menu(
              menu_title=None,
              options=['Albums', 'Top Songs'],
              icons=["file-music-fill","list-ol"],
              orientation="horizontal"
            )
              ...
              for track in top_songs_result['tracks']:
                with st.container():
                  ...
                  with col4:
                    ...
                    similar_songs = st.button('Similar Song', key=track['name'],
